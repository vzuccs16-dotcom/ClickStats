/* ============================================================
   ...
   4. Add your SMP members' YouTube channel IDs below
...
function avatarEl(ch, size = 32) {
  ...
  return `<div class="channel-cell-placeholder" style="width:${size}px;height:${size}px;font-size:${Math.round(size * 0.4)}px">${ch.name[0]}</div>`;
}
...
function populateCompareSelects() {
  ...
  // Default: pick first two different channels
  if (channelData.length >= 2) {
    selA.value = channelData[0].id;
    selB.value = channelData[1].id;
  }
...
function renderCompare() {
  ...
  const rows = stats.map(stat => {
    const aWins = stat.higherWins ? stat.a > stat.b : stat.a < stat.b;
    const bWins = stat.higherWins ? stat.b > stat.a : stat.b < stat.a;
    const tied  = stat.a === stat.b;

    const maxVal = Math.max(stat.a, stat.b, 1);
    const barA = Math.round((stat.a / maxVal) * 100);
    const barB = Math.round((stat.b / maxVal) * 100);

    return `
      <div class="compare-row">
        <div class="compare-side compare-side-a ${aWins ? 'winner' : ''} ${tied ? 'tied' : ''}">
          <span class="compare-value">${stat.fmt(stat.a)}</span>
          <div class="compare-bar-wrap compare-bar-wrap-a">
            <div class="compare-bar compare-bar-a ${aWins ? 'bar-win' : ''}" style="width:${barA}%"></div>
          </div>
        </div>
        ...
        <div class="compare-side compare-side-b ${bWins ? 'winner' : ''} ${tied ? 'tied' : ''}">
          <div class="compare-bar-wrap compare-bar-wrap-b">
            <div class="compare-bar compare-bar-b ${bWins ? 'bar-win' : ''}" style="width:${barB}%"></div>
          </div>
          <span class="compare-value">${stat.fmt(stat.b)}</span>
        </div>
      </div>
    `;
  }).join('');
  ...
  stats.forEach(s => {
    if (s.a === s.b) return;
    if (s.higherWins ? s.a > s.b : s.a < s.b) winsA++;
    else winsB++;
  });

  const verdict = winsA > winsB
    ? `${chA.name} wins ${winsA}–${winsB}`
    : winsB > winsA
      ? `${chB.name} wins ${winsB}–${winsA}`
      : 'All tied up';
