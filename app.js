...
function renderRivalryCard() {
  ...
  // Sort by subs, find the closest pair
  const sorted = [...channelData].sort((a, b) => b.subscribers - a.subscribers);
  let minGap = Infinity, rivalA = null, rivalB = null;

  for (let i = 0; i < sorted.length - 1; i++) {
    const gap = sorted[i].subscribers - sorted[i + 1].subscribers;
    if (gap < minGap) {
      minGap = gap;
      rivalA = sorted[i];
      rivalB = sorted[i + 1];
  ...
  if (!rivalA || !rivalB) { card.style.display = 'none'; return; }

  const totalSubs = rivalA.subscribers + rivalB.subscribers;
  const pctA = Math.round((rivalA.subscribers / totalSubs) * 100);
  const pctB = 100 - pctA;
  const gapStr = fmt(minGap);

  // How close is the gap as a percentage of the leader
  const gapPct = ((minGap / rivalA.subscribers) * 100).toFixed(1);
  const intensity = minGap < 1000 ? 'Neck and neck!' :
                    minGap < 10000 ? 'Very close' :
                    minGap < 50000 ? 'Heating up' :
                    minGap < 200000 ? 'Closing in' : 'Some distance';
    ...
    <div class="rivalry-inner">
      ...
      <div class="rivalry-channels">
        <div class="rivalry-channel rivalry-channel-a">
          ${rivalA.avatar ? `<img src="${rivalA.avatar}" class="rivalry-avatar" />` : `<div class="rivalry-avatar-placeholder">${rivalA.name[0]}</div>`}
        ...
        <div class="rivalry-middle">
          <div class="rivalry-gap-label">${gapStr} apart</div>
          <div class="rivalry-bar-track">
            <div class="rivalry-bar-a" style="width:${pctA}%"></div>
            <div class="rivalry-bar-b" style="width:${pctB}%"></div>
          </div>
          <div class="rivalry-intensity">${intensity} &mdash; ${gapPct}% gap</div>
        </div>
        ...
        <div class="rivalry-channel rivalry-channel-b">
          ...
          ${rivalB.avatar ? `<img src="${rivalB.avatar}" class="rivalry-avatar" />` : `<div class="rivalry-avatar-placeholder">${rivalB.name[0]}</div>`}
        </div>
      </div>
    </div>
  `;
