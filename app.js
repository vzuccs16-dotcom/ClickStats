...
/* ============================================================
   RIVALRY TRACKER
   Finds the two closest channels by subscriber count and
   highlights the gap between them
   ============================================================ */
/* Returns the closest pair by a given stat as { a, b, gap, gapPct } */
function findClosestPair(statFn) {
  const sorted = [...channelData].sort((a, b) => statFn(b) - statFn(a));
  let minGap = Infinity, pairA = null, pairB = null;
  for (let i = 0; i < sorted.length - 1; i++) {
    const gap = statFn(sorted[i]) - statFn(sorted[i + 1]);
    if (gap < minGap) { minGap = gap; pairA = sorted[i]; pairB = sorted[i + 1]; }
  }
  const gapPct = pairA ? (minGap / Math.max(statFn(pairA), 1)) * 100 : 100;
  return { a: pairA, b: pairB, gap: minGap, gapPct };
}

function buildRivalryHTML(rivalA, rivalB, subGap, subGapPct, viewGap, viewGapPct, isHot) {
  const totalSubs  = rivalA.subscribers + rivalB.subscribers;
  const totalViews = rivalA.views + rivalB.views;
  const subPctA    = Math.round((rivalA.subscribers / totalSubs)  * 100);
  const viewPctA   = Math.round((rivalA.views       / totalViews) * 100);

  const subIntensity  = subGapPct  < 1 ? 'Neck and neck!' : subGapPct  < 5 ? 'Very close' : subGapPct  < 15 ? 'Heating up' : 'Closing in';
  const viewIntensity = viewGapPct < 1 ? 'Neck and neck!' : viewGapPct < 5 ? 'Very close' : viewGapPct < 15 ? 'Heating up' : 'Closing in';

  return `
    <div class="rivalry-channels">
      <div class="rivalry-channel rivalry-channel-a">
        ${rivalA.avatar ? `<img src="${rivalA.avatar}" class="rivalry-avatar-lg" />` : `<div class="rivalry-avatar-placeholder rivalry-avatar-lg">${rivalA.name[0]}</div>`}
        <div class="rivalry-ch-info">
          <a href="${rivalA.url}" target="_blank" class="rivalry-name">${rivalA.name}</a>
          <span class="rivalry-subs">${fmt(rivalA.subscribers)} subs</span>
          <span class="rivalry-subs">${fmt(rivalA.views)} views</span>
        </div>
      </div>

      <div class="rivalry-middle">
        ${isHot ? '<div class="rivalry-hot">🔥 HOT RIVALRY</div>' : ''}
        <div class="rivalry-stat-block">
          <div class="rivalry-stat-label">Subscribers — ${fmt(subGap)} apart</div>
          <div class="rivalry-bar-track">
            <div class="rivalry-bar-a" style="width:${subPctA}%"></div>
            <div class="rivalry-bar-b" style="width:${100 - subPctA}%"></div>
          </div>
          <div class="rivalry-intensity">${subIntensity} &mdash; ${subGapPct.toFixed(1)}% gap</div>
        </div>
        <div class="rivalry-stat-block">
          <div class="rivalry-stat-label">Views — ${fmt(viewGap)} apart</div>
          <div class="rivalry-bar-track">
            <div class="rivalry-bar-a" style="width:${viewPctA}%"></div>
            <div class="rivalry-bar-b" style="width:${100 - viewPctA}%"></div>
          </div>
          <div class="rivalry-intensity">${viewIntensity} &mdash; ${viewGapPct.toFixed(1)}% gap</div>
        </div>
      </div>

      <div class="rivalry-channel rivalry-channel-b">
        <div class="rivalry-ch-info rivalry-ch-info-b">
          <a href="${rivalB.url}" target="_blank" class="rivalry-name">${rivalB.name}</a>
          <span class="rivalry-subs">${fmt(rivalB.subscribers)} subs</span>
          <span class="rivalry-subs">${fmt(rivalB.views)} views</span>
        </div>
        ${rivalB.avatar ? `<img src="${rivalB.avatar}" class="rivalry-avatar-lg" />` : `<div class="rivalry-avatar-placeholder rivalry-avatar-lg">${rivalB.name[0]}</div>`}
      </div>
    </div>
  `;
}
...
function renderRivalryCard() {
  const card = document.getElementById('rivalry-card');
  const tab  = document.querySelector('.tab[data-tab="rivalry"]');
  if (channelData.length < 2) { card.style.display = 'none'; return; }

  const subPair  = findClosestPair(c => c.subscribers);
  const viewPair = findClosestPair(c => c.views);

  // Only show as card (not full tab) if BOTH subs AND views are very close (within 10%)
  const bothClose = subPair.gapPct < 10 && viewPair.gapPct < 10 &&
                    subPair.a.id === viewPair.a.id && subPair.b.id === viewPair.b.id;

  if (bothClose) {
    // True rivalry — promote to its own tab
    card.style.display = 'none';
    if (tab) tab.style.display = '';
  } else {
    // Just show mini card, hide full tab
    card.style.display = '';
    if (tab) tab.style.display = 'none';
    if (currentTab === 'rivalry') {
      currentTab = 'overall';
      document.querySelectorAll('.tab').forEach(t => t.classList.toggle('active', t.dataset.tab === 'overall'));
      render();
    }

    // Mini card just shows sub rivalry
    const { a, b, gap, gapPct } = subPair;
    const totalSubs = a.subscribers + b.subscribers;
    const pctA = Math.round((a.subscribers / totalSubs) * 100);
    const intensity = gapPct < 1 ? 'Neck and neck!' : gapPct < 5 ? 'Very close' : gapPct < 15 ? 'Heating up' : gapPct < 30 ? 'Closing in' : 'Some distance';

    card.innerHTML = `
      <div class="rivalry-inner">
        <div class="rivalry-badge">⚔ RIVALRY</div>
        <div class="rivalry-channels">
          <div class="rivalry-channel rivalry-channel-a">
            ${a.avatar ? `<img src="${a.avatar}" class="rivalry-avatar" />` : `<div class="rivalry-avatar-placeholder">${a.name[0]}</div>`}
            <div class="rivalry-ch-info">
              <a href="${a.url}" target="_blank" class="rivalry-name">${a.name}</a>
              <span class="rivalry-subs">${fmt(a.subscribers)} subs</span>
            </div>
          </div>
          <div class="rivalry-middle">
            <div class="rivalry-gap-label">${fmt(gap)} apart</div>
            <div class="rivalry-bar-track">
              <div class="rivalry-bar-a" style="width:${pctA}%"></div>
              <div class="rivalry-bar-b" style="width:${100 - pctA}%"></div>
            </div>
            <div class="rivalry-intensity">${intensity} &mdash; ${gapPct.toFixed(1)}% gap</div>
          </div>
          <div class="rivalry-channel rivalry-channel-b">
            <div class="rivalry-ch-info rivalry-ch-info-b">
              <a href="${b.url}" target="_blank" class="rivalry-name">${b.name}</a>
              <span class="rivalry-subs">${fmt(b.subscribers)} subs</span>
            </div>
            ${b.avatar ? `<img src="${b.avatar}" class="rivalry-avatar" />` : `<div class="rivalry-avatar-placeholder">${b.name[0]}</div>`}
          </div>
        </div>
      </div>
    `;
...
function renderRivalryPage() {
  document.getElementById('rivalry-section').style.display = '';
  const subPair  = findClosestPair(c => c.subscribers);
  const viewPair = findClosestPair(c => c.views);
  const { a, b } = subPair;
  const isHot = subPair.gapPct < 5 && viewPair.gapPct < 5;

  document.getElementById('rivalry-content').innerHTML = buildRivalryHTML(
    a, b, subPair.gap, subPair.gapPct, viewPair.gap, viewPair.gapPct, isHot
  );
}
...
/* ============================================================
   ...
   ============================================================ */
function render() {
  const sections = ['leaderboard-section','milestones-section','frequency-section','compare-section','rivalry-section'];
